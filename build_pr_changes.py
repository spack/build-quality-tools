...
def spawn(command: str, args, logfile=None) -> int:
...
def generate_build_results(installed, passed, failed, about_build_host) -> str:
    ...
    if failed:
        failed_specs = [f[0] for f in failed]
        build_results += f"Failed: {' '.join(failed_specs)}\n"
        for failed_spec, log_file in failed:
            # Add the failed specs and the last lines of the log files to the build results:
            err, stdout, stderr = run(["tail", "-n", "1", log_file])
            if err:
                build_results += f"{failed_spec}: {stderr or stdout}\n"
            else:
                build_results += f"{failed_spec}: {stdout}\n"
...
def check_diff_and_commit(args):
    ...
    while True:
        err, output = subprocess.getstatusoutput("git diff")
        if err:
            print("Failed to get the git diff: ", output)
            return err
        if not output:
            break
        ...
        ret = spawn("git", ["add", "." if args.yes else "-p"])
        if ret:
            print("Failed to add the changes to the index.")
            return ret
    err, output = subprocess.getstatusoutput("git diff --cached")
    if err:
        print("Failed to get the cached git index.", output)
        return err
    if output:
        print(output)
